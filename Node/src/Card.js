...
const cardProto = { // contains data associated with a card
    cardName: null, 
    cost: 0,
    counters: 0,
    quickplay: 0,
    ...
    clickFunct: function(){document.createElement('p')},
    renderObject: function(){
        return (
            <button onClick={this.clickFunct()}>
                <img src={cardImages[cardNames.indexOf(this.cardName)]} width={cardSize}/>
            </button>
            );
    }
};
...
export function getCardByID(id){
    return <img src={cardImages[id]} width={cardSize}/>;
}

export function getCardByName(name){
    return <img src={cardImages[cardNames.indexOf(name)]} width={cardSize}/>;
}
